...
func (g *generator) emitPartial(n *ast.Partial) error {
	...
	if nameExpr.kind == exprString {
		...
		if len(parts) == 2 {
			g.w.line("if err := partials[%q](%s, w); err != nil {", name, partialCtxVar)
		} else {
			g.w.line("if err := render%s(%s, w); err != nil {", goName, partialCtxVar)
		}
		g.w.indentInc()
		g.w.line("return err")
		g.w.indentDec()
	...
	if nameExpr.kind == exprPath {
		if goName, ok := g.partials[nameExpr.value]; ok {
			if len(parts) == 2 {
				g.w.line("if err := partials[%q](%s, w); err != nil {", nameExpr.value, partialCtxVar)
			} else {
				g.w.line("if err := render%s(%s, w); err != nil {", goName, partialCtxVar)
			}
			g.w.indentInc()
	...
	g.w.line("if !ok {")
	g.w.indentInc()
	g.w.line("runtime.MissingPartialOutput(w, %s)", nameVar)
	g.w.indentDec()
	g.w.line("} else if err := partialFn(%s, w); err != nil {", partialCtxVar)
	g.w.indentInc()
	g.w.line("return err")
	g.w.indentDec()
	g.w.line("}")
...
func (g *generator) emitPathValue(path string) string {
	...
	// Parent scope: "../path" or "../" - resolve rest against parent scope
	if path == ".." || strings.HasPrefix(path, "../") {
		...
		parent := g.typedStack[len(g.typedStack)-2]
		if parent.node == nil {
			return "nil"
		}
	...
	// Resolve path: first check if path equals a block param (pathPrefix) in the stack, so "val" finds itemVar when current scope is keyVar
	for i := len(g.typedStack) - 1; i >= 0; i-- {
		s := g.typedStack[i]
