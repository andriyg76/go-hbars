...
// sourceRefComment returns a path safe for "// from path:1" in generated code.
func sourceRefComment(file string) string {
	return commentLine(file)
}

// templateErrorWithFile prepends file path and optional line to err for clearer error messages.
// If err.Error() contains " (line N)", the result is "file:N: message"; otherwise "file: message".
func templateErrorWithFile(file string, err error) error {
	if file == "" {
		return err
	}
	msg := err.Error()
	const linePrefix = " (line "
	if idx := strings.Index(msg, linePrefix); idx >= 0 {
		rest := msg[idx+len(linePrefix):]
		n := 0
		...
		if n > 0 && n < len(rest) && rest[n] == ')' {
			lineStr := rest[:n]
			msgTrim := strings.TrimSpace(msg[:idx])
			return hexerr.New(file + ":" + lineStr + ": " + msgTrim)
		}
	}
	return hexerr.New(file + ": " + msg)
}
...
// When opts.MaxPhase > 0, compilation runs in phases up to MaxPhase and writes
// requested phase outputs (Phase1Output, etc.); returns generated Go when MaxPhase is Phase4.
func CompileTemplates(templates map[string]string, opts Options) ([]byte, error) {
	if opts.MaxPhase > 0 {
		return RunPhased(templates, opts)
	}
	if opts.PackageName == "" {
...
// compileCodegen emits Go source from codegenParams. Used by CompileTemplates and RunPhase4.
func compileCodegen(p codegenParams, opts Options) ([]byte, error) {
	names := p.names
	parsed := p.parsed
	funcNames := p.funcNames
	partialParamTypes := p.partialParamTypes
	canonicalType := p.canonicalType
	primaryCaller := p.primaryCaller
	typeTrees := p.typeTrees
	contextTypeToTemplate := p.contextTypeToTemplate
	useLayoutBlocks := p.useLayoutBlocks
	helperExprs := p.helperExprs
	...
	header.line("// Compiler version: %s", Version)
	...
	// Emit shared partials first so canonical types (e.g. MenuContext) exist before callers reference them.
	var emitOrder []string
	for _, name := range names {
		ownCtx := funcNames[name] + "Context"
		paramType := partialParamTypes[name]
		if paramType != "" && paramType != ownCtx {
			continue
		}
	...
	// Shared partials (templates that are keys of canonicalType) first, then the rest.
	var first, rest []string
	for _, name := range emitOrder {
		if canonicalType[name] != "" {
	...
	// Emit diagram comments for template inclusion and context structure.
	nameSet := make(map[string]bool)
	for _, n := range names {
		nameSet[n] = true
	}
	for _, line := range buildTemplateInheritanceDiagram(emitOrder, typeTrees, nameSet) {
		contextIfaces.line("// %s", commentLine(line))
	}
	contextIfaces.line("//")
	for _, line := range buildContextStructureDiagram(emitOrder, typeTrees, names, funcNames, canonicalType, primaryCaller) {
		contextIfaces.line("// %s", commentLine(line))
	}
	contextIfaces.line("")

	// Emit full type tree for canonical partials that are not root templates (not in names).
	// Their types are referenced by callers; infer type tree from a caller's subtree.
	emitOrderSet := make(map[string]bool)
	for _, n := range emitOrder {
		emitOrderSet[n] = true
	}
	...
	for i, name := range names {
		...
		if file := opts.TemplateFiles[name]; file != "" {
			...
			functions.line("// from %s:1", sourceRefComment(file))
		}
		if useLayoutBlocks {
...
func buildTemplateInheritanceDiagram(emitOrder []string, typeTrees map[string]*typeNode, nameSet map[string]bool) []string {
	...
	for _, name := range emitOrder {
		...
		for _, p := range partials {
			...
			if !seen[edge] {
				seen[edge] = true
				lines = append(lines, "  "+name+" --> "+p)
			}
		}
	}
	if len(seen) == 0 {
		lines = append(lines, "  subgraph roots [roots]")
		for _, name := range emitOrder {
...
func buildContextStructureDiagram(emitOrder []string, typeTrees map[string]*typeNode, names []string, funcNames map[string]string, canonicalType, primaryCaller map[string]string) []string {
	...
	for _, canon := range canonicalType {
		if canon != "" {
			canonicalSet[canon] = true
		}
	}
	var lines []string
	lines = append(lines, "Context type structure:")
	lines = append(lines, "```mermaid")
	lines = append(lines, "classDiagram")
	// Emit root contexts and their nested partial contexts.
	for _, name := range emitOrder {
		goName := funcNames[name]
		if goName == "" {
			continue
		...
		for _, field := range fields {
			...
			if canon != "" {
				lines = append(lines, "  "+rootCtx+" --> "+canon+" : "+goFieldName(field)+"()")
			} else {
				nestedName := contextInterfaceName(goName, field)
				lines = append(lines, "  "+rootCtx+" --> "+nestedName+" : "+goFieldName(field)+"()")
			}
		}
		if len(fields) == 0 && !canonicalSet[rootCtx] {
			lines = append(lines, "  class "+rootCtx+" { }")
		}
	}
	// Emit canonical partial root contexts (shared partials: one type used by multiple callers).
	for _, canon := range canonicalType {
		if canon != "" {
			lines = append(lines, "  class "+canon+" { canonical }")
		}
	}
	lines = append(lines, "```")
	return lines
