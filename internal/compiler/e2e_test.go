...
func TestE2E_UniversalSection(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping e2e test in short mode")
	}
	// Universal section: {{#date}}...{{/date}} and {{#foo}}...{{else}}n{{/foo}} without helpers => section semantics
	templates := map[string]string{
		"main": `{{#date}}{{date}}{{/date}}|{{#foo}}y{{else}}n{{/foo}}`,
	}
	data := map[string]any{"date": "2024-01-15"}
	dataBytes, err := json.Marshal(data)
	if err != nil {
		t.Fatalf("marshal data: %v", err)
	}
	...
	writeFile(filepath.Join(tmpDir, "go.mod"), `module test-universal-section
...
import (
	...
	"os"
	"strings"
	templates "test-universal-section/templates"
)
