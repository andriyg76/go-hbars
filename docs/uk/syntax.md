...
{{> header}}
```
Рендерить партіал `header` з поточним контекстом.

**Партіал з іншим контекстом:**
```handlebars
{{> userCard user}}
```
Рендерить партіал `userCard` з контекстом `user` замість поточного.

**Партіал з локальними змінними:**
```handlebars
{{> footer note="thanks"}}
```
Рендерить партіал `footer`, у якому доступна локальна змінна `note`.
